...
public class Solution {
    ...
    public int dfs(int index, int zeroLeft, int oneLeft, int[][] freqArr, int[][][] dp){
      if(index < 0) return 0;
      else{
        if(dp[index][zeroLeft][oneLeft] != -1) return dp[index][zeroLeft][oneLeft];

        int zeroAtIndex = freqArr[index][0];
        int oneAtIndex = freqArr[index][1];
        int result = (zeroAtIndex > zeroLeft || oneAtIndex > oneLeft) ?
                      dfs(index - 1, zeroLeft, oneLeft, freqArr, dp) :
                      Math.max(dfs(index - 1, zeroLeft, oneLeft, freqArr, dp),
                                                1 + dfs(index - 1, zeroLeft - zeroAtIndex, oneLeft - oneAtIndex, freqArr, dp));
        ...
        for(int j = 0; j < n + 1; j++){
          for(int k = 0; k < l; k++){
            ...
            if(zeroAtIndex > i || oneAtIndex > j){
              ...
              dp[i][j][k] = (k == 0) ? 1 : Math.max(dp[i][j][k-1], dp[i - zeroAtIndex][j - oneAtIndex][k-1] + 1);
            }
          }
        }
      }
