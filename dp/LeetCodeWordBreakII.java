public class Solution {
  public List<String> wordBreak(String s, Set<String> wordDict) {
    // exclude corner case
    if (s.isEmpty() && wordDict.contains(""))
      return null;
    if (s.isEmpty() && !wordDict.contains(""))
      return null;
    ...
    for (int i = 0; i < n; i++)
      prevListList.add(new ArrayList<Integer>());
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = i; j >= 0; j--) {
        ...
        if (wordDict.contains(word) && (j == 0 || dp[j - 1] == true)) {
          dp[i] = true;
          prevListList.get(i).add(j - 1); // add -1 to list if the current index
                                          // is the end of first word
        }
      }
    }
  ...
  public void dfs(List<String> result, List<String> candidate, int index,
                  List<List<Integer>> prevListList, String s) {
    if (index == -1) {
  ...
  public String translate(List<String> candidate) {
    ...
    for (int i = candidate.size() - 1; i >= 0; i--) {
      if (i == candidate.size() - 1)
        result += candidate.get(candidate.size() - 1);
      else {
        result += " ";
