public class Solution {
  public int maxProfit(int[] prices) {
    ...
    for (int i = 0; i < n; i++) {
      if (i == 0)
        prevMinPrice[i] = prices[i];
      else
        prevMinPrice[i] = Math.min(prevMinPrice[i - 1], prices[i]);
    }
    int[] postMaxPrice = new int[n];
    for (int i = n - 1; i >= 0; i--) {
      if (i == n - 1)
        postMaxPrice[n - 1] = prices[n - 1];
      else
        postMaxPrice[i] = Math.max(postMaxPrice[i + 1], prices[i]);
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
      if (i == n - 1)
        postMaxProfit[n - 1] = maxProfitBoughtHere[n - 1];
      else
        postMaxProfit[i] =
            Math.max(postMaxProfit[i + 1], maxProfitBoughtHere[i]);
    }
    // update  result

    int result = 0;
    for (int i = 0; i < n - 1; i++) {
      result = Math.max(result, prevMaxProfit[i] +
                                    postMaxProfit[i + 1]); // two transactions
    }
