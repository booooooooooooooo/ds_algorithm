      ...
      for (int j = 0; j < n; j++) {
        if (charIsMatch(s.charAt(i), p.charAt(j))) {
          ...
          if (charIsMatch(s.charAt(i), p.charAt(j - 1))) {
            dp[i][j] =
                (j - 2 < 0 ? false : dp[i][j - 2]) || dp[i][j - 1] ||
                (i - 1 < 0 ? matchEmpty(p.substring(0, j + 1)) : dp[i - 1][j]);
          } else
...
public class Solution {
  ...
  public boolean dfs(String s, int indexS, String p, int indexP,
                     HashMap<Integer, HashSet<Integer>> visited) {
    if (visited.containsKey(indexS) && visited.get(indexS).contains(indexP))
      ...
      if (!visited.containsKey(indexS))
        visited.put(indexS, new HashSet<Integer>());
